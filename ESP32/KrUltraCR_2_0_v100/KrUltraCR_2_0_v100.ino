...
#define SS_PIN 5
#define RST_PIN 22              // 22 for CRU003 og CRU04, 0 for CRU05 og CRU06
...
// const unsigned long awaitWifiRetryTime = 30000;   // 30000 millisekunder = 30 sekunder
// #define timTick 3
...
//  if (ringBuffer.count() > 0 && WiFi.isConnected() && Firebase.ready()) {       // Elementer i kø som skal overføres til databasen
  if (ringBuffer.count() > 0 && isInternetAvailable() && Firebase.ready()) {       // Elementer i kø som skal overføres til databasen
    leds[1] = CRGB::Purple;
    FastLED.show();
    ...
    if (ringBuffer.count() == 0) {
      Serial.print(".");
      delay (2000);   // vent litt før vi sjekker om det har kommet nye registreringer (ikke bråhast - spar strøm)
    }
...
void setup() {
  ...
  xTaskCreatePinnedToCore (         // definerer at dbUpdate skal kjøre på core 0
    ...
    12000,      // Stack size in words. 10000 ok, 5000 for lite
